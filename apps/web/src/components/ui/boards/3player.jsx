...
export default function ThreePlayerBoard({
  ...
  // Create square lookup and grid - 3 player is 4 player minus the left side
  let square_lookup = {}
  const squares = []
  ...
  for (let r = 0; r < boardSize; r++) {
    ...
    for (let f = 0; f < boardSize; f++) {
      // 3-player cross: vertical arm (f 3-10) + horizontal arm (r 3-10) MINUS left extension (f 0-2, r 3-10)
      const inVerticalArm = f >= 3 && f <= 10
      const inHorizontalArm = r >= 3 && r <= 10
      const inLeftExtension = f <= 2 && r >= 3 && r <= 10
      
      if ((inVerticalArm || inHorizontalArm) && !inLeftExtension) {
  ...
  return <div className="chess-board-3p" style={{ '--grid-size': boardSize }}>
    <div className="chess-squares-3p">
      {squares.flat().map((square, i) => {
        if (!square.valid) {
          return <div key={i} className="chess-square-invalid"></div>
        }
        
        let classes = ['chess-square-3p']
